...
if ($env=getenv('PHP_PEAR_INSTALL_DIR')) {
    ...
    putenv('PHP_PEAR_INSTALL_DIR=C:\1_2_3\xampp\htdocs\code\2\libs/PEAR');
}

if ($env=getenv('PHP_PEAR_BIN_DIR')) {
    define("PHP_PEAR_BIN_DIR",$env);
} else {
    putenv('PHP_PEAR_BIN_DIR=C:\1_2_3\xampp\htdocs\code\2\libs');
}
if ($env=getenv('PHP_PEAR_PHP_BIN')) {
    define("PHP_PEAR_PHP_BIN",$env);
} else {
    putenv('PHP_PEAR_PHP_BIN=C:\1_2_3\xampp\htdocs\code\2\libs\');
}
...
ini_set('include_path', 'C:\1_2_3\xampp\htdocs\code\2\libs/PEAR');
$useDHTML         = true;
